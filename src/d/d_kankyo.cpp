        ...
        for (u16 i = 0; i < modelData->getMaterialNum(); i++) {
            ...
            if (mat_name[3] == 'M' && mat_name[4] == 'A') {
                ...
                if (memcmp(&mat_name[3], "MA10", 4) == 0 || memcmp(&mat_name[3], "MA02", 4) == 0) {
                    ...
                    // CLEANUP this session (VR water-reflection investigation,
                    // resolved 2026-07-29): this branch used to carry a long
                    // trail of read-only TEV-stage-inspection logging (rounds
                    // 8/9/10/11) and a magenta/blend-override diagnostic
                    // (rounds 4-7) from the original "water renders solid
                    // black in VR" investigation. Both are removed now that
                    // the bug is fixed by an unrelated mechanism entirely
                    // (skipping this material's draw altogether in VR --
                    // d_com_inf_game.cpp's dComIfGd_drawXluListInvisible()/
                    // drawOpaListInvisible()) -- none of what this diagnostic
                    // code found or attempted ended up load-bearing for the
                    // actual fix. See CLAUDE.md section 3 for the full
                    // history if it's ever useful again.
