...
struct ViewProjection {
    ...
    void apply(J3DTexMtxInfo* info) const {
        Mtx projection;
        // Use dComIfGd_getReflectionFovAspect() rather than
        // dComIfGd_getView()->fovy/aspect directly -- those are deliberately
        // never updated for VR (see its own comment, d_com_inf_game.cpp),
        // so building this env-map matrix straight from them left it using
        // the stale flatscreen camera while actually rendering VR's much
        // wider eye, pushing reflection texture coordinates entirely
        // outside the texture (root-caused as "water renders solid black
        // in VR").
        f32 fovy, aspect;
        dComIfGd_getReflectionFovAspect(&fovy, &aspect);
        C_MTXLightPerspective(projection, fovy, aspect, scaleS, scaleT, transS, transT);
